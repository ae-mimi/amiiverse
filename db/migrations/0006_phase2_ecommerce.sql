...
SET
    product_id = REPLACE(id, '_default', ''),
    sku = (
        SELECT pc.slug || '-default'
        FROM products_cache pc
        WHERE pc.id || '_default' = product_variants.id
    ),
    title = (
        SELECT pc.title
        FROM products_cache pc
...
SET updated_at = (
    SELECT pc.updated_at
    FROM products_cache pc
    WHERE pc.id || '_default' = inventory.variant_id
)
WHERE variant_id IN (
    SELECT id || '_default'
    FROM products_cache
