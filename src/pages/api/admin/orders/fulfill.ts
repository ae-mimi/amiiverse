...
export const POST: APIRoute = async ({ request, locals }) => {
    ...
    if (!canTransitionOrderStatus(from, "fulfilled")) {
        return jsonResponse({ error: `Cannot fulfill from status ${from}` }, 409);
    }
    ...
    return jsonResponse({ ok: true, reference, status: "fulfilled" });
};
