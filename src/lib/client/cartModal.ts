import { cartStore, type ClientCart, type ClientCartItem } from "../cart/cartStore";
...
class CheckoutManager {
    ...
    async init() {
        ...
        window.addEventListener("open-checkout", (event: Event) => {
            const customEvent = event as CustomEvent<Record<string, unknown>>;
            void this.open(customEvent.detail);
        });
        ...
        document.addEventListener("keydown", (event) => {
            if (event.key === "Escape" && this.modal.classList.contains("isOpen")) {
                this.close();
            }
        });
        this.cartItemsWrap.addEventListener("click", (event) =>
    ...
    getLineItemTitle(item: ClientCartItem): string {
        return (item.title as string) || "Product";
    }

    getLineItemType(item: ClientCartItem): string {
        const metadata = item.metadata as Record<string, unknown> | undefined;
        const rawType = metadata?.productType || metadata?.type || "physical";
    ...
    getLineItemImage(item: ClientCartItem): string {
        ...
        return (item.thumbnail as string) || (product?.thumbnail as string) || "";
    }
    ...
    async handleCartActions(event: Event) {
        ...
        if (removeId) {
            this.renderCart(await cartStore.removeLineItem(removeId));
            return;
        }
        ...
        if (incId) {
            ...
            if (!item) return;
            this.renderCart(
                await cartStore.updateQuantity(incId, Number(item.quantity || 1) + 1),
            );
            return;
        }
        ...
        if (decId) {
            ...
            if (!item) return;
            this.renderCart(
                await cartStore.updateQuantity(decId, Number(item.quantity || 1) - 1),
            );
        }
    }

    async open(data: Record<string, unknown>) {
    ...
    async handleSubmit(event: Event) {
        ...
        const errorEl = this.modal.querySelector("#modal-error") as HTMLElement;
        errorEl.hidden = true;
...
export function initCheckoutModal(): void {
    if (typeof document === "undefined" || cartModalInitialized) return;
    const modal = document.getElementById("checkout-modal");
    if (!modal) return;
    cartModalInitialized = true;
    const manager = new CheckoutManager(modal);
    void manager.init();
}
